...
import com.tcl.dias.networkaugmentation.service.NwaEorEquipDetailsService;
...
@RestController
public class NwaEorEquipDetailsController {

    @Autowired
    NwaEorEquipDetailsService nwaEorEquipDetailsService;
    ...
    @PostMapping(value = "saveDeviceDetails",produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseResource<NwaEorEquipDetailsBean> saveDeviceDetails(@RequestBody NwaEorEquipDetailsBean nwaEorEquipDetails){
        NwaEorEquipDetailsBean  response=nwaEorEquipDetailsService.saveNwaEorEquipDetails(nwaEorEquipDetails);
   ...
   /* @ApiResponses(value = { @ApiResponse(code = 200, message = Constants.SUCCESS, response = NwaEorEquipDetails.class),
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = Constants.SUCCESS, response = NwaEorEquipDetails.class),
            ...
            @ApiResponse(code = 417, message = Constants.EXCEPTION_FAILED) })
    @PutMapping(value = "/putDeviceDetails/{orderCode}",produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseResource<NwaEorEquipDetailsBean> getNwaOrderDetails(@PathVariable("orderCode") String orderCode ){
        NwaEorEquipDetailsBean response = nwaEorEquipDetailsService.getNwaEorEquipDetail(orderCode);
        return new ResponseResource<>(ResponseResource.R_CODE_OK, ResponseResource.RES_SUCCESS, response,
                Status.SUCCESS);
