package com.tcl.dias.networkaugmentation.specification;
		...
		return new Specification<ScServiceDetail>() {
			...
			public Predicate toPredicate(Root<ScServiceDetail> root, CriteriaQuery<?> query,
				...
				if (status != null && !status.isEmpty()) {
					...
					predicates
							.add(mstStatusJoin.get("code").in(java.util.Arrays.asList("INPROGRESS","HOLD","CANCELLED","ACTIVE","INACTIVE")));
				}

				if (orderType != null && !orderType.isEmpty()) {
					predicates.add(scOrderJoin.get("orderType").in(orderType));
				}
				...
				if (orderCategory != null && !orderCategory.isEmpty()) {
					...
					predicates.add(scOrderJoin.get("orderCategory").in(orderCategory));
