 ...
 */
@Component("setCLRDelegate")
public class SetCLRDelegate implements JavaDelegate {
    private static final Logger logger = LoggerFactory.getLogger(SetCLRDelegate.class);
    public static final String SET_CLR_SUCCESS = "setCLRSyncSuccess";

    @Autowired
	MQUtils mqUtils;

	@Value("${queue.setclrsync}")
	String setCLRSyncQueue;
	...
	public void execute(DelegateExecution execution) {
        logger.info("SetCLRDelegate  invoked for {} ", execution.getCurrentActivityId());
        String errorMessage="";
		String errorCode="";
        String req = "";
        Task task = null;
        try {
            ...
            SetCLRSyncBean setCLRSyncBean = getSetCLRSyncBean(taskService.getRandomNumberForCrammer()+execution.getProcessInstanceId()+"#"+"CLR"+"#"+task.getServiceCode(),
                    task.getServiceCode());
            setCLRSyncBean.setServiceId(serviceId);

            req = Utils.convertObjectToJson(setCLRSyncBean);

            String setCLRSyncResponse = (String) mqUtils.sendAndReceive(setCLRSyncQueue, req);
            logger.info("setCLRSyncResponse => {} ", setCLRSyncResponse);

            if (StringUtils.isBlank(setCLRSyncResponse)) {
                execution.setVariable(SET_CLR_SUCCESS, false);
                ...
                if (response.getStatus()!=null && response.getStatus() ) {
                    execution.setVariable(SET_CLR_SUCCESS, true);
                } else {
                    execution.setVariable(SET_CLR_SUCCESS, false);
			...
			if (scServiceDetail.isPresent()&& StringUtils.isNotBlank(errorMessage)) {
				...
				try {
					...
					componentAndAttributeService.updateAdditionalAttributes(scServiceDetail.get(),
							...
							AttributeConstants.ERROR_MESSAGE, "set-clr");
    ...
    private SetCLRSyncBean getSetCLRSyncBean(String processInstanceId, String serviceCode) {
        ...
        setCLRSyncBean.setInitialRelationship("ISSUED");
        setCLRSyncBean.setFinalRelationship("ACTIVE");
