 ...
 */
@Component("stagePlanStartDelegate")
public class StagePlanStartDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(StagePlanStartDelegate.class);
	...
	public void execute(DelegateExecution execution) {
		...
		if (execution.getCurrentActivityId() != null) {
			logger.info("StagePlanStartListener invoked for {} ", execution.getCurrentActivityId());
			
			logger.info("StagePlanStartListener invoked for {} id: {} ProcessInstanceId:{}  EventName:{} ",execution.getCurrentActivityId(),
			...
			try {
				projectPlanHelperService.processProjectPLan(processsType, "stagePlan", execution, preceders);
			} catch (Exception e) {
				e.printStackTrace();	
