 ...
 */
@Component("releaseDummyWANIPDelegate")
public class ReleaseDummyWANIPDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(ReleaseDummyWANIPDelegate.class);

	@Autowired
	MQUtils mqUtils;
	@Value("${queue.releasedummywanipsync}")
	String releaseDummyWANIPSyncQueue;
	...
	public void execute(DelegateExecution execution) {
		logger.info("releaseDummyWANIPDelegate invoked for {} ", execution.getCurrentActivityId());
		String errorMessage = "";
		String errorCode="";
		...
		try {
            ...
            IPServiceSyncBean ipServiceSyncBean = getReleaseDummyIPServiceBean(taskService.getRandomNumberForCrammer()+execution.getProcessInstanceId(), processMap);

            String req = Utils.convertObjectToJson(ipServiceSyncBean);
            logger.info("releaseDummyWANIPRequest {} ", req);

			String assignDummyWANIPSyncResponse = (String) mqUtils.sendAndReceive(releaseDummyWANIPSyncQueue, req,90000);
			logger.info("releaseDummyWANIPSyncResponse= {} ", assignDummyWANIPSyncResponse);
			if (StringUtils.isBlank(assignDummyWANIPSyncResponse)) {
                ...
                if(response.getStatus()!=null && response.getStatus()) {                  
                    execution.setVariable("releaseDummyWANIPDummySyncCallSuccess", true);
                }else{
                    execution.setVariable("releaseDummyWANIPDummySyncCallSuccess", false);
            ...
            logger.error("releaseDummyWANIPDelegate Exception", e);
            execution.setVariable("releaseDummyWANIPDummySyncCallSuccess", false);
		...
		if (StringUtils.isNotBlank(message)) {			
				try {
					logger.info("releaseDummyWANIPDelegate error log started");

					componentAndAttributeService.updateAdditionalAttributes(task.getServiceId(),
							"serviceDesignReleaseIpInfoCallFailureReason",
							componentAndAttributeService.getErrorMessageDetails(errorMessage, errorCode),
							AttributeConstants.ERROR_MESSAGE, "release-dummy-wan-ip");

				} catch (Exception e) {
					logger.error("releaseDummyWANIPDelegate getting error message details----------->{}", e);
				}		
	 ...
	 */
	private IPServiceSyncBean getReleaseDummyIPServiceBean(String processInstanceId, Map<String, Object> processMap) {
		...
		ipServiceSyncBean.setWanIPAddress("");
		return ipServiceSyncBean;
	}
