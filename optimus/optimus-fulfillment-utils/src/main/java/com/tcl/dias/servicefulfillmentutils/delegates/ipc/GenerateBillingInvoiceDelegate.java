 ...
 */
@Component("generateBillingInvoiceDelegate")
public class GenerateBillingInvoiceDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(GenerateBillingInvoiceDelegate.class);

	
    @Autowired
	WorkFlowService workFlowService;    
   
    @Value("${queue.invoiceGenSync}")
	...
	public void execute(DelegateExecution execution) {
		logger.info("GenerateBillingInvoiceDelegate for IPC invoked for {} id={}", execution.getCurrentActivityId(),execution.getId());
		try {
