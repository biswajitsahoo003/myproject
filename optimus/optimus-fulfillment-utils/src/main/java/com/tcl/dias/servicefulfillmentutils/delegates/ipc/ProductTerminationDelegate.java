 ...
 */
@Component("productTerminationDelegate")
public class ProductTerminationDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(ProductTerminationDelegate.class);

	
    @Autowired
	WorkFlowService workFlowService;    
   
    @Value("${queue.productTermSync}")
		...
		try {
			...
			if(!StringUtils.isBlank(status) && (status.equals("Success") || status.equals(IpcConstants.NO_RECORD_FOUND)))
			{
				...
				if(status.equals(IpcConstants.NO_RECORD_FOUND)) {
                	execution.setVariable("skipProductTerminationAsync", true);
                } else {
                	execution.setVariable("skipProductTerminationAsync", false);
                }
			}
			else {
