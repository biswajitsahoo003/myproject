 ...
 */
@Component("nwTerminateBillingInvoiceDelegate")
public class NetworkTerminateBillingInvoiceDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(NetworkTerminateBillingInvoiceDelegate.class);

	
    @Autowired
	WorkFlowService workFlowService;    
   
    @Value("${queue.invoice.termination.lr}")
	...
	public void execute(DelegateExecution execution) {
		logger.info("NetworkTerminateBillingInvoiceDelegate invoked for {} id={}", execution.getCurrentActivityId(),execution.getId());
		try {
            ...
            logger.info("NetworkTerminateBillingInvoiceDelegate completed");
            workFlowService.processServiceTaskCompletion(execution);
			
		} catch (Exception e) {
			logger.error("Exception in NetworkTerminateBillingInvoiceDelegate{}", e);
		}
