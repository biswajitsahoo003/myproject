 ...
 */
@Component("nwAccountCreationDelegate")
public class NetworkAccountCreationDelegate implements JavaDelegate {
	private static final Logger logger = LoggerFactory.getLogger(NetworkAccountCreationDelegate.class);

	@Autowired
	WorkFlowService workFlowService;

	@Value("${queue.accountCreateSync.network}")
	...
	public void execute(DelegateExecution execution) {
		logger.info("AccountCreationDelegate invoked for {} id={}", execution.getCurrentActivityId(),
				execution.getId());
		try {
			...
			if (!StringUtils.isBlank(status) && status.equals("Success")) {
				...
				execution.setVariable("accountCreationErrorMsg", status.split("Success|Fail")[1]);
			}
