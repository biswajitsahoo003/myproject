...
import com.tcl.dias.servicefulfillmentutils.objectcreator.ObjectCreator;
...
//		.save(new com.tcl.dias.servicefulfillment.entity.entities.Stage());
	...
	//	map.put("key", objectCreator.getMstProcessDef().getMstStageDef().getKey() );
		...
		Mockito.when(processPlanRepository.findByServiceIdAndMstProcessDefKey(Mockito.anyInt(), Mockito.any(), "A")).thenReturn(objectCreator.getProcessPlan());
	...
	public void testProcessActivity() throws TclCommonException {
		...
		Mockito.when(activityPlanRepository.findByServiceIdAndMstActivityDefKey(Mockito.anyInt(),Mockito.any(), "A"))
