...
public class IzosdwanPricingCustomServiceJobs implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		try {
			LOGGER.info("Inside execute method of CUSTOM IzosdwanPricingServiceJobs!!");
			...
			List<IzosdwanPricingService> izosdwanPricingServices = izosdwanPricingServiceRepository.findPricingServicesDetailsByStatus(CommonConstants.NEW_STATUS,CommonConstants.CUSTOM);
			if(izosdwanPricingServices!=null && !izosdwanPricingServices.isEmpty()) {
				LOGGER.info("Inside if of CUSTOM IzosdwanPricingServiceJobs!!");
				LOGGER.info("Got new records for processing CUSTOM pricing services !!");
				...
				LOGGER.info("Inside IzosdwanPricingServiceJobs before CUSTOM queue call!!");
				mqUtils.send("rabbitmq.oms.izosdwan.quote.price.custom.service", Utils.convertObjectToJson(izosdwanPricingServices));
				LOGGER.info("Inside IzosdwanPricingServiceJobs after CUSTOM queue call!!");
			}
		} catch (Exception e) {
	...
	public void updateIzosdwanPricingServiceStatus(String status,List<IzosdwanPricingService> izosdwanPricingServices,IzosdwanPricingServiceRepository izosdwanPricingServiceRepository) {
		LOGGER.info("Updating the pricing service status to CUSTOM {}",status);
