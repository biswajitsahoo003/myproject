...
/**
 * 
 * This file contains the OdrWebexMapper.java class.
 * 
 *
...
@Component
public class OdrWebexMapper {

	private static final Logger LOGGER = LoggerFactory.getLogger(OdrWebexMapper.class);
	...
	public OdrOrderBean mapOrderEntityToBean(OdrOrder odrOrder, List<OdrContractInfo> odrContractInfoEntity,
			List<OdrOrderAttribute> odrOrderAttributeEntity, List<OdrServiceDetail> odrServiceDetailEntity,
			List<com.tcl.dias.oms.entity.entities.OdrServiceCommercial> odrServiceCommercials,List<OdrSolutionComponent> odrSolutionComponents, List<OdrWebexServiceCommercial> odrWebexServiceCommercials) {
		
		OdrOrderBean mapOrderEntityToBean = mapOrderEntityToBean(odrOrder, odrContractInfoEntity, odrOrderAttributeEntity, odrServiceDetailEntity,
				odrServiceCommercials, odrSolutionComponents);
		
		Set<OdrWebexCommercialBean> odrWebexCommercialBeans = new HashSet<>();
		if(odrWebexServiceCommercials!=null && !odrWebexServiceCommercials.isEmpty()) {
			odrWebexServiceCommercials.stream().forEach(odrSolutionComponent->{
				odrWebexCommercialBeans.add(mapOdrWebexCommercialToBean(odrSolutionComponent));
			});
		}
		
		mapOrderEntityToBean.setOdrWebexCommercialBean(odrWebexCommercialBeans);
		
		return mapOrderEntityToBean;
	}
	
	public OdrWebexCommercialBean mapOdrWebexCommercialToBean(OdrWebexServiceCommercial odrWebexServiceCommercial) {
		OdrWebexCommercialBean odrWebexCommercialBean = new OdrWebexCommercialBean();
		odrWebexCommercialBean.setServiceId(odrWebexServiceCommercial.getOdrServiceDetail().getId());
		odrWebexCommercialBean.setComponentName(odrWebexServiceCommercial.getComponentName());
		odrWebexCommercialBean.setComponentDesc(odrWebexServiceCommercial.getComponentDesc());
		odrWebexCommercialBean.setComponentType(odrWebexServiceCommercial.getComponentType());
		odrWebexCommercialBean.setHsnCode(odrWebexServiceCommercial.getHsnCode());
		odrWebexCommercialBean.setQuantity(odrWebexServiceCommercial.getQuantity());
		odrWebexCommercialBean.setArc(odrWebexServiceCommercial.getArc());
		odrWebexCommercialBean.setUnitMrc(odrWebexServiceCommercial.getUnitMrc());
		odrWebexCommercialBean.setMrc(odrWebexServiceCommercial.getMrc());
		odrWebexCommercialBean.setNrc(odrWebexServiceCommercial.getNrc());
		odrWebexCommercialBean.setUnitNrc(odrWebexServiceCommercial.getUnitNrc());
		odrWebexCommercialBean.setTcv(odrWebexServiceCommercial.getTcv());
		odrWebexCommercialBean.setCiscoUnitListPrice(odrWebexServiceCommercial.getCiscoUnitListPrice());
		odrWebexCommercialBean.setCiscoDiscountPercent(odrWebexServiceCommercial.getCiscoDiscountPercent());
		odrWebexCommercialBean.setCiscoUnitNetPrice(odrWebexServiceCommercial.getCiscoUnitNetPrice());
		odrWebexCommercialBean.setContractType(odrWebexServiceCommercial.getContractType());
		odrWebexCommercialBean.setEndpointManagementType(odrWebexServiceCommercial.getEndpointManagementType());
		odrWebexCommercialBean.setIsActive(odrWebexServiceCommercial.getIsActive());
		odrWebexCommercialBean.setShortText(odrWebexServiceCommercial.getShortText());
