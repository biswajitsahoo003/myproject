...
/**
 * This file contains the OdrMapper.java class.
 * 
 *
 * @author Manojkumar R
 ...
 */
@Component
public class OdrMapper {

	private static final Logger LOGGER = LoggerFactory.getLogger(OdrMapper.class);
	@Autowired
	...
	public OdrOrderBean mapOrderEntityToBean(OdrOrder odrOrder, List<OdrContractInfo> odrContractInfoEntity,
		...
		odrOrderBean.setErfCustPartnerLeId(odrOrder.getErfCustPartnerLeId());
		odrOrderBean.setPartnerCuid(odrOrder.getPartnerCuid());
		...
		odrOrderBean.setCancellationOrderCode(odrOrder.getCancelledParentOrderCode());
		...
		if (odrOrder.getSiteLevelBilling() != null) {
			if (odrOrder.getSiteLevelBilling().equalsIgnoreCase("Y")) {
				odrOrderBean.setSiteBillingFlag(true);
			}
		}
		
		//mapOdrAsset(odrAssets, odrServiceDetailBeans);
		return odrOrderBean;
	}
	...
	public OdrServiceDetailBean mapServiceEntityToBean(OdrServiceDetail odrServiceDetail) {
		...
		if (odrServiceDetail.getTerminationEffectiveDate() != null) {
			odrServiceDetailBean.setTerminationEffectiveDate(
					DateUtil.convertDateToString(odrServiceDetail.getTerminationEffectiveDate()));
		}
		odrServiceDetailBean.setEtcValue(odrServiceDetail.getEtcValue() != null && odrServiceDetail.getEtcValue() > 0
				? odrServiceDetail.getEtcValue().toString()
				: "0.00");
		odrServiceDetailBean.setEtcWaiver(odrServiceDetail.getEtcWaiver());
		odrServiceDetailBean.setTerminationReason(odrServiceDetail.getTerminationReason());
		odrServiceDetail.setNegotiationRequired(odrServiceDetail.getNegotiationRequired());
		...
		odrServiceDetailBean.setMasterVrfServiceId(odrServiceDetail.getMasterServiceId());
		odrServiceDetailBean.setIsMultiVrf(odrServiceDetail.getIsMultiVrf());
		...
		odrServiceDetailBean.setRrfsDate(odrServiceDetail.getRrfsDate());
		...
		odrServiceDetailBean.setIsAmended(odrServiceDetail.getIsAmended());
		odrServiceDetailBean.setAmendedServiceId(odrServiceDetail.getAmendedServiceId());
		...
		if(odrServiceDetail.getOdrOrder().getOrderType()!=null && "CANCELLATION".equalsIgnoreCase(odrServiceDetail.getOdrOrder().getOrderType())){
			LOGGER.info("Inside cancellation flow in batch job for O2C from L2O order code : {}",odrServiceDetail.getOdrOrder().getUuid());
			odrServiceDetailBean.setCancellationCharges(odrServiceDetail.getCancellationCharges());
			odrServiceDetailBean.setAbsorbedOrPassedOn(odrServiceDetail.getAbsorbedOrPassedOn());
		}
		/*
		 * for (OdrServiceCommercialComponent odrServiceCommercialComponent :
		 * odrServiceDetail .getOdrServiceCommercialComponent()) {
		 ...
		 */
		/*List<OdrAsset> assets=odrAssetRepository.findByOdrServiceDetail(odrServiceDetail);
		List<OdrAssetBean> assetBeans=new ArrayList<>();
		for(OdrAsset asset: assets)
		{
			OdrAssetBean assetBean=new OdrAssetBean();
			assetBean.setCreatedBy(asset.getCreatedBy());
			assetBean.setCreatedDate(asset.getCreatedDate());
			assetBean.setFqdn(asset.getFqdn());
			assetBean.setId(asset.getId());
			assetBean.setIsActive(asset.getIsActive());
			assetBean.setIsSharedInd(asset.getIsSharedInd());
			assetBean.setName(asset.getName());
			assetBean.setOriginnetwork(asset.getOriginnetwork());
			assetBean.setOdrServiceDetailId(asset.getOdrServiceDetail().getId());
			assetBean.setPublicIp(asset.getPublicIp());
			assetBean.setUpdatedBy(asset.getUpdatedBy());
			assetBean.setUpdatedDate(asset.getUpdatedDate());
			assetBean.setType(asset.getType());
			assetBeans.add(assetBean);
		}
		odrServiceDetailBean.setOdrAssetBeans(assetBeans);*/
		List<OdrAssetBean> assetBeans=new ArrayList<>();
		if(odrServiceDetail.getOdrAssets() != null && !odrServiceDetail.getOdrAssets().isEmpty()) {
			for(OdrAsset odrAsset : odrServiceDetail.getOdrAssets()) {
				OdrAssetBean mapOdrAsset = mapOdrAsset(odrAsset);
				assetBeans.add(mapOdrAsset);
			}
		}
		odrServiceDetailBean.setOdrAssetBeans(assetBeans);
		odrServiceDetailBean.setMultiVrfSolution(odrServiceDetail.getMultiVrfSolution());
		odrServiceDetailBean.setIntlDestinationCntryCode(odrServiceDetail.getIntlDestinationDialCode());
		return odrServiceDetailBean;
	}
	
	public void mapOdrAsset(List<OdrAsset> odrAssets, Set<OdrServiceDetailBean> odrServiceDetailBeans) {
		if(odrAssets != null && !odrAssets.isEmpty()) {
			HashMap<Integer, List<OdrAssetBean>> tmpMapper = new HashMap<>();
			for(OdrAsset odrAsset : odrAssets) {
				OdrAssetBean mapOdrAsset = mapOdrAsset(odrAsset);
				if(tmpMapper.get(mapOdrAsset.getOdrServiceDetailId()) == null)
					tmpMapper.put(mapOdrAsset.getOdrServiceDetailId(), new ArrayList<OdrAssetBean>());
				tmpMapper.get(mapOdrAsset.getOdrServiceDetailId()).add(mapOdrAsset);
			}
			odrServiceDetailBeans.forEach((OdrServiceDetailBean odrServiceDetailBean) -> {
				odrServiceDetailBean.setOdrAssetBeans(tmpMapper.get(odrServiceDetailBean.getId()));
	        });
		}
	}
	
	public OdrAssetBean mapOdrAsset(OdrAsset odrAsset) {
		OdrAssetBean odrAssetBean = new OdrAssetBean();
		odrAssetBean.setId(odrAsset.getId());
		odrAssetBean.setFqdn(odrAsset.getFqdn());
		odrAssetBean.setIsActive(odrAsset.getIsActive());
		odrAssetBean.setIsSharedInd(odrAsset.getIsSharedInd());
		odrAssetBean.setName(odrAsset.getName());
		odrAssetBean.setOriginnetwork(odrAsset.getOriginnetwork());
		odrAssetBean.setOdrServiceDetailId(odrAsset.getOdrServiceDetail().getId());
		odrAssetBean.setPublicIp(odrAsset.getPublicIp());
		odrAssetBean.setCreatedBy(odrAsset.getCreatedBy());
		odrAssetBean.setCreatedDate(DateUtil.convertTimestampToDate(odrAsset.getCreatedDate()));
		odrAssetBean.setUpdatedBy(odrAsset.getUpdatedBy());
		odrAssetBean.setUpdatedDate(DateUtil.convertTimestampToDate(odrAsset.getUpdatedDate()));
		odrAssetBean.setType(odrAsset.getType());
		if(odrAsset.getOdrAssetAttributes() != null)
			for(OdrAssetAttribute odrAssetAttribute : odrAsset.getOdrAssetAttributes()) {
				odrAssetBean.addOdrAssetAttributeBean(mapOdrAssetAttribute(odrAssetAttribute));
			}
		if(odrAsset.getOdrAssetRelations() != null)
			for(OdrAssetRelation odrAssetRelation : odrAsset.getOdrAssetRelations()) {
				odrAssetBean.addOdrAssetRelationBean(mapOdrAssetRelation(odrAssetRelation));
			}
		return odrAssetBean;
	}
	
	public OdrAssetAttributeBean mapOdrAssetAttribute(OdrAssetAttribute odrAssetAttribute) {
		OdrAssetAttributeBean odrAssetAttributeBean = new OdrAssetAttributeBean();
		odrAssetAttributeBean.setId(odrAssetAttribute.getId());
		odrAssetAttributeBean.setAttributeAltValueLabel(odrAssetAttribute.getAttributeAltValueLabel());
		odrAssetAttributeBean.setAttributeName(odrAssetAttribute.getAttributeName());
		odrAssetAttributeBean.setAttributeValue(odrAssetAttribute.getAttributeValue());
		odrAssetAttributeBean.setCategory(odrAssetAttribute.getCategory());
		odrAssetAttributeBean.setCreatedBy(odrAssetAttribute.getCreatedBy());
		odrAssetAttributeBean.setIsActive(odrAssetAttribute.getIsActive());
		odrAssetAttributeBean.setUpdatedBy(odrAssetAttribute.getUpdatedBy());
		odrAssetAttributeBean.setUpdatedDate(odrAssetAttribute.getUpdatedDate());
		return odrAssetAttributeBean;
	}
	
	public OdrAssetRelationBean mapOdrAssetRelation(OdrAssetRelation odrAssetRelation) {
		OdrAssetRelationBean odrAssetRelationBean = new OdrAssetRelationBean();
		odrAssetRelationBean.setId(odrAssetRelation.getId());
		odrAssetRelationBean.setOdrAssetId(odrAssetRelation.getOdrAsset().getId());
		odrAssetRelationBean.setOdrRelatedAssetId(odrAssetRelation.getOdrRelatedAsset().getId());
		odrAssetRelationBean.setBusinessRelationName(odrAssetRelation.getBusinessRelationName());
		odrAssetRelationBean.setEndDate(odrAssetRelation.getEndDate());
		odrAssetRelationBean.setIsActive(odrAssetRelation.getIsActive());
		odrAssetRelationBean.setRelationPort(odrAssetRelation.getRelationPort());
		odrAssetRelationBean.setRelationResiliency(odrAssetRelation.getRelationResiliency());
		odrAssetRelationBean.setRelationType(odrAssetRelation.getRelationType());
		odrAssetRelationBean.setRemarks(odrAssetRelation.getRemarks());
		odrAssetRelationBean.setStartDate(odrAssetRelation.getStartDate());
		odrAssetRelationBean.setCreatedBy(odrAssetRelation.getCreatedBy());
		odrAssetRelationBean.setCreatedDate(odrAssetRelation.getCreatedDate());
		odrAssetRelationBean.setUpdatedBy(odrAssetRelation.getUpdatedBy());
		odrAssetRelationBean.setUpdatedDate(odrAssetRelation.getUpdatedDate());
