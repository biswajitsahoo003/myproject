...
import com.tcl.dias.common.gsc.beans.WholesaleTigerNotificationBean;
...
/**
 * This file contains the polling job to pick Wholesale Tiger service API call records and process accordingly
 *
...
public class WholesaleTigerServiceJob extends QuartzJobBean {

    private static final Logger LOGGER = LoggerFactory.getLogger(WholesaleTigerServiceJob.class);
    ...
    private static List<String> WHOLESALE_JOB_STATUSES = ImmutableList.of(SfdcServiceStatus.INPROGRESS.toString(),
            SfdcServiceStatus.FAILURE.toString());

    private boolean isJobStruck(ThirdPartyServiceJob job, Map<String, ThirdPartyServiceJob> inProgressWholesaleJobs) {
        ThirdPartyServiceJob inProgressJob = inProgressWholesaleJobs.get(jobUid(job));
    ...
    private static boolean isJobInProgress(ThirdPartyServiceJob job, Map<String, ThirdPartyServiceJob> inProgressWholesaleJobs) {
        ThirdPartyServiceJob inProgressJob = inProgressWholesaleJobs.get(jobUid(job));
        return Objects.nonNull(inProgressJob)
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) {
        try {
            List<ThirdPartyServiceJob> wholesaleServiceJobs = thirdPartyServiceJobsRepository.findWholesaleTigerByServiceStatusBySeq(
                    SfdcServiceStatus.NEW.toString());
            if (!CollectionUtils.isEmpty(wholesaleServiceJobs)) {
                LOGGER.info(String.format("%s new wholesale service jobs found", wholesaleServiceJobs.size()));
            }
            Map<String, ThirdPartyServiceJob> inProgressWholesaleJobs = thirdPartyServiceJobsRepository.findByServiceStatusInAndThirdPartySource(
                    WHOLESALE_JOB_STATUSES, ThirdPartySource.WHOLESALE_TIGER_ORDER.toString())
                    .stream()
                    .collect(Collectors.toMap(WholesaleTigerServiceJob::jobUid, Function.identity(), (first, second) -> second));
            if (!CollectionUtils.isEmpty(inProgressWholesaleJobs)) {
                LOGGER.info(String.format("%s in-progress wholesale service jobs found", inProgressWholesaleJobs.size()));
            }

            List<ThirdPartyServiceJob> struckJobs = findStruckJobs(inProgressWholesaleJobs.values());
            struckJobs.forEach(struckJob -> {
                LOGGER.info(String.format("Marking wholesale service job %s with id %s as %s", jobUid(struckJob), struckJob.getId(),
                        SfdcServiceStatus.STRUCK.toString()));
                thirdPartyServiceJobDao.updateStruckStatusByServiceType(struckJob.getRefId(),
                        ThirdPartySource.WHOLESALE_TIGER_ORDER.toString(),
                        struckJob.getServiceType());
            });

            List<ThirdPartyServiceJob> retryAbleJobs = findRetryableJobs(inProgressWholesaleJobs.values());
            wholesaleServiceJobs.addAll(retryAbleJobs);
            wholesaleServiceJobs.forEach(job -> {
                String jobUID = jobUid(job);
                if (isJobInProgress(job, inProgressWholesaleJobs)) {
                    LOGGER.info(String.format("Wholesale service job %s with id %s is in progress", jobUID, job.getId()));
                } else if (isJobStruck(job, inProgressWholesaleJobs)) {
                    if (Objects.nonNull(job.getRefId())) {
                        ...
                        WholesaleTigerNotificationBean bean = new WholesaleTigerNotificationBean();
                        ...
                        try {
                            mqUtils.send("rabbitmq_wholesale_error_queue", toJson(bean));
                        } catch (TclCommonException e) {
                            LOGGER.warn("error in processing to queue call for wholesale service job{}", e);
                        }

                    }
                    LOGGER.info(String.format("Marking Wholesale service job %s with id %s as %s", jobUID, job.getId(),
                            SfdcServiceStatus.STRUCK.toString()));
                    thirdPartyServiceJobDao.updateStruckStatusByServiceType(job.getRefId(),
                            ThirdPartySource.WHOLESALE_TIGER_ORDER.toString(), job.getServiceType());
                } else {
                    LOGGER.info(String.format("Starting wholesale service job %s with id %s retry count %s", jobUID, job.getId(),
                            job.getRetryCount()));
                    try {
                    ...
                    LOGGER.info(String.format("Started wholesale service job %s with id %s successfully", jobUID, job.getId()));
                }
            });
        } catch (Exception e) {
            LOGGER.warn(String.format("Error occurred while executing Wholesale Service Job: %s",
