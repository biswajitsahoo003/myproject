 ...
 */
@Component("activityEndListener")
public class ActivityEndListener implements ExecutionListener {
	private static final long serialVersionUID = -3288118379238635451L;
	private static final Logger logger = LoggerFactory.getLogger(ActivityEndListener.class);
	...
	public void notify(DelegateExecution execution) {
		logger.info("ActivityEndListener invoked for {} id: {} ProcessInstanceId:{}  EventName:{} ",execution.getCurrentActivityId(),
				execution.getId(),execution.getProcessInstanceId(),execution.getEventName());
		
		workFlowService.processActivityCompletion(execution);		
	}
