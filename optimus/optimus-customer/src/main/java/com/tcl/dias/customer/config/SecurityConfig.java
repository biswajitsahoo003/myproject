package com.tcl.dias.customer.config;
...
public class SecurityConfig extends KeycloakWebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
				...
				.antMatchers("/v1/cof/**").hasAnyAuthority(ObjectArrays.concat(UserActionsConstants.CUSTOMER_ACCESS_GROUP,
				UserActionsConstants.PARTNER_ACCESS_GROUP, String.class))
				.antMatchers("/isv/**").hasAnyAuthority(ObjectArrays.concat(UserActionsConstants.INTERNAL_ACCESS_GROUP,
						UserActionsConstants.OB_GROUP, String.class))
				.antMatchers("/v1/**").hasAnyAuthority(ArrayUtils.addAll(ArrayUtils.addAll(UserActionsConstants.CUSTOMER_ACCESS_GROUP,
				UserActionsConstants.USER_ACCESS_GROUP), UserActionsConstants.PARTNER_ACCESS_GROUP))
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/actuator/**", "/v2/api-docs", "/swagger**", "/swagger/**", "/v2/customers/**","/health/**");
	}
