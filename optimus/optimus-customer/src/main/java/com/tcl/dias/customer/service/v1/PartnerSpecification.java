...
/**
 * 
 * This is the specification class for Partner
 * 
 *
 * @author AnandhiV
...
public class PartnerSpecification {
	 ...
	 * @return
	 */
	public static Specification<Partner> getAllCustomers(final String name) {
		return new Specification<Partner>() {
			...
			public Predicate toPredicate(Root<Partner> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
				...
				if(StringUtils.isNoneEmpty(name)) {
					predicates.add(builder.like(builder.lower(root.get("name")), "%"+StringUtils.lowerCase(name)+"%"));
				}
