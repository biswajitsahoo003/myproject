...
/**
 * This file contains the IllSlaService.java class.
 * 
...
@Service
@Transactional
public class IllSlaService {

	private static final Logger LOGGER = LoggerFactory.getLogger(IllSlaService.class);
	...
	public void constructIllSiteSla(QuoteIllSite quoteIllSite, ProductSlaBean productBean) throws TclCommonException {
		...
		if (serviceVariant != null && CommonConstants.ECONET.equalsIgnoreCase(serviceVariant)) {
			persistSlaDetail(quoteIllSite, CommonConstants.ECONET_SLA, FPConstants.NETWORK_UPTIME.toString());
			persistSlaDetail(quoteIllSite, CommonConstants.ECONET_SLA,
					FPConstants.PACKET_DROP_WITHOUT_PERCENTAGE.toString());
		} else {
	...
	private void persistPacketDrop(QuoteIllSite quoteIllSite) {
		...
		List<QuoteProductComponent> quoteComponents = quoteProductComponentRepository
				.findByReferenceIdAndMstProductComponent_NameAndReferenceName(quoteIllSite.getId(), "IAS Common",QuoteConstants.ILLSITES.toString());
	...
	private String getServiceVariant(QuoteIllSite quoteIllSite) {
		...
		List<QuoteProductComponent> quoteProductComponentList = quoteProductComponentRepository
				.findByReferenceIdAndMstProductComponent_NameAndType(quoteIllSite.getId(), OrderDetailsExcelDownloadConstants.IAS_COMMON,
