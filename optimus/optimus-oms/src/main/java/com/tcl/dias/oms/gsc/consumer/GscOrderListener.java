...
public class GscOrderListener {
	...
	public boolean checkSfdcStageForOrder(String orderCode) {
		String sfdcStage = gscOrderService.getSfdcStageForOrder(orderCode);
		return SFDCConstants.CLOSED_WON_COF_RECI.equalsIgnoreCase(sfdcStage);
	}

	@RabbitListener(queuesToDeclare = { @Queue("${rabbitmq.gsc.order.process.queue}") })
		...
		try {
			...
			if (checkSfdcStageForOrder(job.getRefId())) {
				...
				case TIGER_SERVICE_TYPE_INTERNATIONAL_ORDER:
					...
					LOGGER.info("Tiger International order Management Request :: {}", orderManagementRequest.toString());
					Integer orderId = gscOrderManagementRequestBuilder.getOrderId(orderManagementRequest);
					executeAndHandleOrderManagementResponse(
							() -> tigerServiceHelper.placeInternationalOrder(orderManagementRequest, orderId), job,
							orderId, TIGER_SERVICE_TYPE_INTERNATIONAL_ORDER);
					break;
				case TIGER_SERVICE_TYPE_DOMESTIC_ORDER:
					...
					executeAndHandleOrderManagementResponse(() -> tigerServiceHelper
							.placeDomesticOrder(domesticOrderManagementRequest, domesticOrderId), job, domesticOrderId,
				...
				case TIGER_SERVICE_TYPE_INTERCONNECT_ORDER_ACCESS_SERVICES:
					...
					executeAndHandleOrderManagementResponse(() -> tigerServiceHelper
							.placeInterConnectOrder(interConnectOrderManagementRequest, interConnectOrderId), job, interConnectOrderId,
				...
				case TIGER_SERVICE_TYPE_INTERCONNECT_ORDER_GLOBAL_OUTBOUND:
					...
					executeAndHandleOrderManagementResponse(() -> tigerServiceHelper
									.placeInterConnectOrder(interConnectGlobalOutboundOrderManagementRequest, interConnectGlobalOutboundOrderId), job, interConnectGlobalOutboundOrderId,
				...
				case TIGER_SERVICE_TYPE_INTERCONNECT_ORDER_DOMESTIC_VOICE_NVT:
					...
					executeAndHandleOrderManagementResponse(() -> tigerServiceHelper
									.placeInterConnectOrder(interConnectDomesticNVTOrderManagementRequest, interConnectDomesticNVTOrderId), job, interConnectDomesticNVTOrderId,
				...
				case TIGER_SERVICE_TYPE_INTERCONNECT_ORDER_DOMESTIC_VOICE_VTS:
					...
					executeAndHandleOrderManagementResponse(() -> tigerServiceHelper
									.placeInterConnectOrder(interConnectDomesticVTSOrderManagementRequest, interConnectDomesticVTSOrderId), job, interConnectDomesticVTSOrderId,
	...
	@RabbitListener(queuesToDeclare = {@Queue("${rabbitmq.gsc.tiger.order.change.outpulse.process.queue}")})
