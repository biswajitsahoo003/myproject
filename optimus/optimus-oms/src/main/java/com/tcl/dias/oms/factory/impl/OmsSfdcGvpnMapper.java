 ...
 */
@Component
public class OmsSfdcGvpnMapper extends OmsAbstractSfdcHandler implements IOmsSfdcInputHandler {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(OmsSfdcGvpnMapper.class);
	...
	public OpportunityBean getOpportunityBean(QuoteToLe quoteToLe, OpportunityBean opportunityBean)
		...
		opportunityBean.setSelectProductType(SFDCConstants.GVPN.toString());
		if (MACDConstants.MACD_QUOTE_TYPE.equalsIgnoreCase(quoteToLe.getQuoteType())) {
			...
			if (MACDConstants.REQUEST_TERMINATION_SERVICE.equalsIgnoreCase(quoteToLe.getQuoteCategory())) {
				...
				LOGGER.info("Macd Details : {}", macdDetail);
				...
				// //customerMailReceivedDate is specific for termination request
			...
			} else if (MACDConstants.ADD_SITE_SERVICE.equalsIgnoreCase(quoteToLe.getQuoteCategory())) {
...
//					opportunityBean.setPreviousMRC(StringUtils.EMPTY);
				...
				//opportunityBean.setMainVPNIdC(quoteToLe.getErfServiceInventoryTpsServiceId());
				if(serviceDetailsList != null && !serviceDetailsList.isEmpty()) {
					opportunityBean.setMainVPNIdC(serviceDetailsList.get(0).getTpsServiceId());
				}
				opportunityBean.setRecordType("Renewal/Change Order/Termination");
				opportunityBean.setCurrentCircuitServiceId(null);
			//	opportunityBean.setPreviousMRC("0");
				LOGGER.info("Add site scenario's Record type is -----> {} , The Current service Id is ----> {} for Main VPN circuit  -----> {} " , opportunityBean.getRecordType(),
						opportunityBean.getCurrentCircuitServiceId() , opportunityBean.getMainVPNIdC());
				//opportunityBean.setPreviousNRC("0");
//				opportunityBean.setDifferentialMRCAutoC();
		...
		LOGGER.info("Input  for create Opportunity GVPN {}", opportunityBean);
	...
	public ProductServiceBean getProductServiceInput(QuoteToLe quoteToLe, ProductServiceBean productServiceBean,
		...
		//set type value as GVPN instead of Global VPN as per shreyas mail multi vrf 
		productServiceBean.setType(SFDCConstants.GVPN.toString());
		...
		productServiceBean.setOfPortsC("2");
		productServiceBean.setCloudEnablementC("No");
		if(quoteToLe.getIsMultiVrf()!=null) {
		  productServiceBean.setMultiVRFSolution(quoteToLe.getIsMultiVrf());
		}
		...
		if (MACDConstants.MACD_QUOTE_TYPE.equalsIgnoreCase(quoteToLe.getQuoteType())) {
			if (MACDConstants.REQUEST_TERMINATION_SERVICE.equalsIgnoreCase(quoteToLe.getQuoteCategory())) {
				...
				productServiceBean.setType(SFDCConstants.LAYER3MANAGEDMPLSVPN);
				productServiceBean.setOrderType(MACDConstants.ADDITION_OF_SITE);
				productServiceBean.setOpportunityId(quoteToLe.getTpsSfdcOptyId());
			} else if (MACDConstants.SHIFT_SITE_SERVICE.equalsIgnoreCase(quoteToLe.getQuoteCategory())) {
				productServiceBean.setType(SFDCConstants.LAYER3MANAGEDMPLSVPN);
        ...
        productServiceBean.setOrderType(SFDCConstants.NEW_ORDER);
			...
			//added for multi vrf macd default type gvpn for multi vrf yes
			LOGGER.info("multi vrf flag"+quoteToLe.getIsMultiVrf());
			if(quoteToLe.getIsMultiVrf()!=null) {
				if(quoteToLe.getIsMultiVrf().equalsIgnoreCase("Yes")) {
					productServiceBean.setType(SFDCConstants.GVPN.toString());
				}
				
			}
	
		LOGGER.info("Input  for product Service GVPN {}", productServiceBean);
	...
	public ProductServiceBean updateProductServiceInput(QuoteToLe quoteToLe, ProductServiceBean productServiceBean,
		...
		productServiceBean.setRecordTypeName("Global VPN");
		//productServiceBean.setType("Global VPN");
		//set type value as GVPN instead of Global VPN as per shreyas mail multi vrf 
		productServiceBean.setType(SFDCConstants.GVPN.toString());
		productServiceBean.setProductType(SFDCConstants.GVPN.toString());
		productServiceBean.setOfPortsC("2");
		...
		if (quoteToLe.getTermInMonths() != null) {
			...
			LOGGER.info("Final set value for term in months in proposal sent block is ----> {} " , productServiceBean.getTermInMonthsC());
					...
					if(serviceDetail != null && serviceDetail.getServiceDetailBeans() != null && !serviceDetail.getServiceDetailBeans().isEmpty()) {
						...
						if(optimusMdmDetailBean.getSiteCode() != null && optimusMdmDetailBean.getOrderCode() != null 
							...
							if(quoteSiteDifferentialCommercialList != null && !quoteSiteDifferentialCommercialList.isEmpty()) {
								...
								quoteSiteDifferentialCommercialList = gvpnQuoteService.persistQuoteSiteCommercialsAtServiceIdLevel(quoteIllSitesList.get(0), quoteIllSitesList.get(0).getProductSolution().getQuoteToLeProductFamily().getQuoteToLe(), orderIllSitesList.get(0));
								quoteSiteDifferentialCommercialList.stream().forEach(differentialCommercial -> {
									diffMrc[0] += differentialCommercial.getDifferentialMrc();
									diffNrc[0] += differentialCommercial.getDifferentialNrc();
								});
								...
								LOGGER.info("site code or order code is null");
...
//								productServiceBean.setProductNRC((optimusMdmDetailBean.getNrc() != null) ? Double.valueOf(0-optimusMdmDetailBean.getNrc()) : 0);
								...
								try {
									...
									LOGGER.info("POS GVPN Fetching  service details for {} ",optimusMdmDetailBean.getServiceId());
		...
		LOGGER.info("Input  for update  product Service GVPN {}", productServiceBean);
	...
	public UpdateOpportunityStage getOpportunityUpdate(QuoteToLe quoteToLe,
		...
		if(MACDConstants.MACD_QUOTE_TYPE.equalsIgnoreCase(quoteToLe.getQuoteType()))
		{
			...
			if (Objects.nonNull(quote)) {
				...
				LOGGER.info("Opportunity COPF Parent id is :::: {} ",opportunityBean.getCopfIdC());
...
public FeasibilityRequestBean getFeasibilityBean(Integer quoteIllSiteId,
    ...
    LOGGER.info("Initiating get Feasibility Bean : getting vpn port");
	...
	LOGGER.info("Initiating get Feasibility Bean : getting port bandwidth");
	...
	LOGGER.info("Initiating get Feasibility Bean : getting interface");
//	feasibilityBean.setPortCircuitCapacityC(quotatt.get(0).getAttributeValues() + " Mbps");
	...
	LOGGER.info("Initiating get Feasibility Bean : getting lastmile");
	List<QuoteProductComponent> quoteProductComponents1 = quoteProductComponentRepository
			.findByReferenceIdAndMstProductComponent_NameAndType(
					...
					"primary");
	LOGGER.info("Initiating get Feasibility Bean : getting local loop bandwidth");
	...
	LOGGER.info("Initiating get Feasibility Bean : site feasibility quoteIllsiteId {}",quoteIllSiteId);
...
//	}
	List<SiteFeasibility> primarySiteFeasibilities = siteFeasibilityRepository.findByQuoteIllSite_IdAndType(quoteIllSiteId, "primary");
	String feasiblityResponseJson;

	if (!primarySiteFeasibilities.isEmpty()) {
		SiteFeasibility primarySiteFeasibility = primarySiteFeasibilities.stream().findFirst().get();
		if (Objects.nonNull(primarySiteFeasibility.getResponseJson())) {
			feasiblityResponseJson = primarySiteFeasibility.getResponseJson();

			if (primarySiteFeasibility.getFeasibilityMode().equalsIgnoreCase(INTL)) {
				IntlNotFeasible intlNotFeasible = (IntlNotFeasible) Utils.convertJsonToObject(feasiblityResponseJson, IntlNotFeasible.class);
				feasibilityBean.setOtherPOPAEndC(intlNotFeasible.getxConnectAllSitesLong() != null ? intlNotFeasible.getxConnectAllSitesLong() : SFDCConstants.INTL_DEFAULT_POP_NAME);
			} else {
		...
		LOGGER.info("Primary Site Feasibilities are empty for quoteIllSiteId {}", quoteIllSiteId);
	}
	...
	// added for multi vrf
	QuoteToLe quoteToLe=quoteIllSite.get().getProductSolution().getQuoteToLeProductFamily().getQuoteToLe();
		if (quoteToLe != null && quoteToLe.getIsMultiVrf() != null) {
			if (quoteToLe.getIsMultiVrf().equalsIgnoreCase("Yes")) {
				String isMultiVrf = null;
				String noOfVrf = null;
				QuoteProductComponent quoteProductComponent = null;
				List<QuoteProductComponent> prodComponent = null;
				prodComponent = quoteProductComponentRepository.findByReferenceIdAndMstProductComponent_NameAndType(
						quoteIllSite.get().getId(), CommonConstants.VRF_COMMON, "primary");
				if (prodComponent != null) {
					quoteProductComponent = prodComponent.get(0);
					List<QuoteProductComponentsAttributeValue> quotMultiVrfAttribute = quoteProductComponentsAttributeValueRepository
							.findByQuoteProductComponent_IdAndProductAttributeMaster_Name(quoteProductComponent.getId(),
									CommonConstants.MULTI_VRF);
					if (!quotMultiVrfAttribute.isEmpty()) {
						if (!quotMultiVrfAttribute.get(0).getAttributeValues().isEmpty()) {
							isMultiVrf = quotMultiVrfAttribute.get(0).getAttributeValues();
						}
					}
					List<QuoteProductComponentsAttributeValue> quoteNoOfVrfAttribute = quoteProductComponentsAttributeValueRepository
							.findByQuoteProductComponent_IdAndProductAttributeMaster_Name(quoteProductComponent.getId(),
									CommonConstants.NO_OF_VRFS);
					if (!quoteNoOfVrfAttribute.isEmpty()) {
						if (!quoteNoOfVrfAttribute.get(0).getAttributeValues().isEmpty()) {
							noOfVrf = quoteNoOfVrfAttribute.get(0).getAttributeValues();
						}
					}
					LOGGER.info("multi vrf get Feasibility Bean atribute" + isMultiVrf + "noOfVrf:" + noOfVrf);
					feasibilityBean.setIsMultiVrf(isMultiVrf);
					feasibilityBean.setNoOfVrf(noOfVrf);
				}
				
			}
		}
	
	LOGGER.info("Exiting get Feasibility Bean");
