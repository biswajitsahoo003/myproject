...
@Service
public class GscProductCatalogService2 {

	private static final Logger LOGGER = LoggerFactory.getLogger(GscProductCatalogService2.class);
	 ...
	 * @return {@link PdfOutboundContext}
	 */
	private PdfOutboundContext getQuoteForSurcharge(PdfOutboundContext context) throws TclCommonException {
	 ...
	 */
	private QuoteToLe getQuoteToLe(String quoteCode) throws TclCommonException {
		return Optional.ofNullable(quoteToLeRepository.findByQuote_QuoteCode(quoteCode)).map(quoteToLes -> {
			...
			return quoteToLe;
		}).orElseThrow(() -> new TclCommonException(String.format("Quote with code  %s not found", quoteCode),
	...
	private PdfOutboundContext saveOutboundSurchargePrices(PdfOutboundContext context)
		...
		context.surchargePricingBean = getOutboundSurchargePrices();
		return context;
	}
	 ...
	 * @throws IllegalArgumentException
	 */
	private List<GscOutboundSurchargePricingBean> getOutboundSurchargePrices()
			throws TclCommonException, IllegalArgumentException {
		...
		return gscOutboundSurchargePricingBeans;
	}
	...
	private PdfOutboundContext createSurchargePDFFile(PdfOutboundContext surchargeContext)
		...
		Map variable = objectMapper.convertValue(
				ImmutableMap.of("gscOutboundSurchargePricingBean", surchargeContext.surchargePricingBean), Map.class);
	...
	private void setAttachmentForObjectStorage(PdfOutboundContext context, String objectStorageUrl,
											   String updatedFileName) throws TclCommonException {
		...
		if (attachmentId != null && attachmentId != -1) {
			List<OmsAttachment> omsAttachments = omsAttachmentRepository
					.findByQuoteToLeAndAttachmentTypeAndReferenceName(context.quoteToLe, GscConstants.OTHERS,
	...
	private PdfOutboundContext saveGeneratedFile(PdfOutboundContext context) throws IOException, TclCommonException {
		...
		if (swiftApiEnabled.equalsIgnoreCase("true")) {
			StoredObject storedObject = fileStorageService.uploadFiles(context.fileName, "GSIP_OUTBOUND_PRICE_FILES",
					context.targetStream);
	 ...
	 */
	@Transactional
	public GscOutboundPricesDownloadBean generateAndSaveSurchargeOutboundPrices(String quoteCode,
																				HttpServletResponse response) throws TclCommonException, IOException, DocumentException {
		PdfOutboundContext context = createSurchargeOutboundContext(quoteCode, response);
		getQuoteForSurcharge(context);
		saveOutboundSurchargePrices(context);
		createSurchargePDFFile(context);
		saveGeneratedFile(context);
		return context.downloadBean;
	}
	 ...
	 * @return {@link PdfOutboundContext}
	 */
	private PdfOutboundContext getQuoteForOutbound(PdfOutboundContext context) throws TclCommonException {
		context.quoteToLe = getQuoteToLe(context.quoteCode);
	...
	private List<GscOutboundPriceBean> getOutboundPrices(String dynamicColoumnName) throws TclCommonException {
		...
		if (StringUtils.isNotBlank(response)) {
			...
			throw new TclCommonException(OUTBOUND_MQ_ERROR, ResponseResource.R_CODE_NOT_FOUND);
		}
	 ...
	 * @throws TclCommonException
	 */
	private PdfOutboundContext saveOutboundPrices(PdfOutboundContext context) throws TclCommonException {
	...
	public GscOutboundPricesDownloadBean generateAndSaveOutboundPrices(String quoteCode, HttpServletResponse response)
		...
		PdfOutboundContext context = createOutboundContext(quoteCode, response);
		getQuoteForOutbound(context);
		saveOutboundPrices(context);
		createOutboundPDFFile(context);
		saveGeneratedFile(context);
		return context.downloadBean;
	}
	 ...
	 * @return
	 */
	private Workbook createOutboundWorkbook(Workbook workbook, List<GscOutboundPriceBean> gscOutboundPricingBeans,
											String coloumnName) {
	...
	private PdfOutboundContext createOutboundExcelFile(PdfOutboundContext context) throws IOException {
		...
		createOutboundWorkbook(workbook, context.outBoundPdfRequest.getGscOutboundPriceBean(),
				context.dynamicColoumnName);
	 ...
	 */
	@Transactional
	public GscOutboundPricesDownloadBean generateAndSaveOutboundPricesExcel(String quoteCode,
																			HttpServletResponse response) throws TclCommonException, IOException {
		PdfOutboundContext context = createOutboundContextExcel(quoteCode, response);
		getQuoteForOutbound(context);
		saveOutboundPrices(context);
		createOutboundExcelFile(context);
		saveGeneratedFile(context);
		return context.downloadBean;
	}
	 ...
	 */
	@Transactional
	public HttpServletResponse downloadOutboundSurchargePrices(HttpServletResponse response)
			throws TclCommonException, IOException, DocumentException {
		List<GscOutboundSurchargePricingBean> prices = getOutboundSurchargePrices();
		return downloadOutboundSurchargePdf(prices, response);
	}
	 ...
	 * @throws TclCommonException
	 */
	private HttpServletResponse downloadOutboundSurchargePdf(
		...
		response.setHeader("Content-Disposition",
				"attachment; filename=\"" + GscAttachmentTypeConstants.GSC_SURCHARGE_PRICES_PDF + "\"");
		try {
