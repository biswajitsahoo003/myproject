...
public class GscProductCatalogService {
	...
	public GscOutboundPricesDownloadBean generateAndSaveSurchargeOutboundPrices(String quoteCode, HttpServletResponse response) {
		...
		/*PdfOutboundContext exclctx = Try.success(createSurchargeOutboundExcelContext(quoteCode,response))
				.map(this::getQuoteForSurcharge)
				.mapTry(this::saveOutboundSurchargePrices)
				.mapTry(this::createSurchargeExcelFile)
				.mapTry(this::saveGeneratedFile).get();*?
		/*end*/
		
		/*PdfOutboundContext pdfCtx =  Try.success(createSurchargeOutboundContext(quoteCode,response))
				.map(this::getQuoteForSurcharge).get();
		pdfCtx.surchargePricingBean = exclctx.surchargePricingBean;
		
		return 	Try.success(pdfCtx)
	...
	private PdfOutboundContext createSurchargeOutboundExcelContext(String quoteCode, HttpServletResponse response) {
		PdfOutboundContext context = new PdfOutboundContext();
		context.quoteCode = quoteCode;
		context.response = response;
		context.downloadBean = new GscOutboundPricesDownloadBean();
		context.referenceName = GscConstants.GSIP_SURCHARGE_EXCEL;
		context.fileName=GscAttachmentTypeConstants.GSC_SURCHARGE_PRICES_EXCL;
		return context;
	}

	private PdfOutboundContext createSurchargeExcelFile(PdfOutboundContext surchargeContext) throws TclCommonException, IOException {
		InputStream inputStream=new ByteArrayInputStream(excelGeneratorService.generateExcelByteArrayFromBean(null, surchargeContext.surchargePricingBean));
	...
	@Transactional
	public GscOutboundPricesDownloadBean generateAndSaveSurchargeOutboundPricesInExcel(String quoteCode, HttpServletResponse response) {
		return Try.success(createSurchargeOutboundExcelContext(quoteCode,response))
				.map(this::getQuoteForSurcharge)
				.mapTry(this::saveOutboundSurchargePrices)
				.mapTry(this::createSurchargeExcelFile)
				.mapTry(this::saveGeneratedFile)
