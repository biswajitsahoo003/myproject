...
@Component
public class OmsCancellationIzopcMapper implements OmsCancellationHandler {

	private static final Logger LOGGER = LoggerFactory.getLogger(OmsCancellationIzopcMapper.class);
	...
	public List<QuoteProductComponent> createQuoteProductComponent(QuoteIllSite illSite,
		...
		if (orderIllSite != null) {
			...
			List<OrderProductComponent> orderProductComponents = orderProductComponentRepository
					.findByReferenceIdInAndReferenceName(orderIllSite.getId(), QuoteConstants.IZO_PC_SITES.toString());
	...
	private QuoteProductComponent constructQuoteProductComponent(QuoteIllSite illSite,
		...
		productComponent.setReferenceName(QuoteConstants.IZO_PC_SITES.toString());
		quoteProductComponentRepository.save(productComponent);
	...
	private Set<QuoteProductComponentsAttributeValue> constructQuoteAttributes(
		...
		if (ordrProductComponentsAttributeValues != null) {
			...
			attributesList.add(PDFConstants.CLOUD_PROVIDER);
			attributesList.add(PDFConstants.VPN_NAME);
			attributesList.add(PDFConstants.BANDWIDTH);
			attributesList.add(PDFConstants.SERVICE_ID);
			attributesList.add(PDFConstants.AVAILABILITY);
			attributesList.add(PDFConstants.TYPE_OF_PEERING);
			attributesList.add(PDFConstants.CLOUD_PROVIDER_REF_ID);
			attributesList.add(PDFConstants.NO_OF_VPC_VNET);
			attributesList.add(PDFConstants.TOPOLOGY);
			attributesList.add(PDFConstants.SITE_TYPE);
			attributesList.add(OrderDetailsExcelDownloadConstants.PORT_TYPE);
			attributesList.add(PDFConstants.COS_PROFILE);
			attributesList.add(PDFConstants.CSP_PROVIDED_VLAN_ID);
			attributesList.add(PDFConstants.WAN_IP_ADDRESS);

			if (PDFConstants.IZO_PORT.equalsIgnoreCase(quoteProductComponent.getMstProductComponent().getName())) {
				attributesList.stream().forEach(attribute -> {
					QuoteProductComponentsAttributeValue quoteAttributeValue = new QuoteProductComponentsAttributeValue();
					LOGGER.info("attribute {}", attribute);
					switch (attribute) {
					case (PDFConstants.CLOUD_PROVIDER):
						quoteAttributeValue.setProductAttributeMaster(getProductAttributeMaster(attribute));
					...
					case PDFConstants.SITE_TYPE:
						break;
					case OrderDetailsExcelDownloadConstants.PORT_TYPE:
						break;
					case PDFConstants.COS_PROFILE:
						break;
					case PDFConstants.CSP_PROVIDED_VLAN_ID:
						break;
					case PDFConstants.WAN_IP_ADDRESS:
						break;
					}
	...
	private List<OrderProductComponent> constructOrderProductComponent(Integer id, OrderIllSite illSite) {
		...
		List<QuoteProductComponent> productComponents = quoteProductComponentRepository
				.findByReferenceIdAndReferenceName(id, QuoteConstants.IZO_PC_SITES.toString());
