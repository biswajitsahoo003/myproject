...
//
	...
	public GscContactAttributeInfo getContractInfo(Integer quoteId, Integer quoteToLeId) {
		...
		try{
			QuoteToLe quoteToLe = quoteToLeRepository.findById(quoteToLeId).orElseThrow(() -> new NullPointerException("QuoteToLeId not found"));
			Integer customerLegalEntityId = quoteToLe.getErfCusCustomerLegalEntityId();
			// Enterprise sales - get customer le contact details.
			if (INTERNAL_USERS.toString().equalsIgnoreCase(userInfoUtils.getUserType()) && customerLegalEntityId!=null) {
				CustomerLeContactDetailBean customerLeContactDetailBean = getCustomerLeContact(customerLegalEntityId).get();
				attributeInfo.setFirstName(customerLeContactDetailBean.getName());
				attributeInfo.setContactNo(customerLeContactDetailBean.getMobilePhone());
				attributeInfo.setEmailId(customerLeContactDetailBean.getEmailId());
				attributeInfo.setAddress(customerLeContactDetailBean.getAddress());
			} else if(PartnerConstants.PARTNER.equalsIgnoreCase(userInfoUtils.getUserType())) {
				Map<String, Object> actions = (Map<String, Object>) userInfoUtils.getUserInformation().getActions();
				String classification = quoteToLe.getClassification();
				...
				if (actions.containsKey(optMasterPartnerPSamAction) || SELL_WITH_CLASSIFICATION.equalsIgnoreCase(classification)) {
					...
					// partner Sell through - Show logged in User details
					extractQuoteLeAttributes(quoteId, quoteToLeId, attributeInfo);
				}
			}else{
				//enterprise - show logged in user details
				extractQuoteLeAttributes(quoteId, quoteToLeId, attributeInfo);
			}
		}
		catch (TclCommonException e){
			LOGGER.error(">> Error in partner getContractInfo");
		}
		return attributeInfo;
	}

	public GscContactAttributeInfo extractQuoteLeAttributes (Integer quoteId, Integer quoteToLeId, GscContactAttributeInfo attributeInfo){
