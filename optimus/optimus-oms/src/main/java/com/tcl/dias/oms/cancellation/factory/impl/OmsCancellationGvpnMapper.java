...
@Component
public class OmsCancellationGvpnMapper implements OmsCancellationHandler {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(OmsCancellationGvpnMapper.class);
	...
	public List<QuoteProductComponent> createQuoteProductComponent(QuoteIllSite illSite,
		...
		if (orderIllSite != null) {
			...
			List<OrderProductComponent> orderProductComponents = orderProductComponentRepository
					.findByReferenceIdInAndReferenceName(orderIllSite.getId(), QuoteConstants.GVPN_SITES.toString());
			if (orderProductComponents != null) {
				for (OrderProductComponent orderProductComponent : orderProductComponents) {
					...
					productComponent.setQuoteProductComponentsAttributeValues(
							constructQuoteAttributes(attributes, productComponent, illSite.getProductSolution()
									.getQuoteToLeProductFamily().getQuoteToLe().getQuote().getId(), null));
			...
			QuoteProductComponent productComponent = constructQuoteProductComponent(illSite, serviceDetailBean,
					ComponentConstants.VPN_PORT.getComponentsValue());
			constructQuoteComponentPrice(null, productComponent);
			productComponent.setQuoteProductComponentsAttributeValues(
					constructQuoteAttributes(null, productComponent, illSite.getProductSolution().getQuoteToLeProductFamily().getQuoteToLe().getQuote().getId(), serviceDetailBean));
			quoteProductComponents.add(productComponent);
			...
			productComponentLastMile.setQuoteProductComponentsAttributeValues(
					constructQuoteAttributes(null, productComponentLastMile, illSite.getProductSolution().getQuoteToLeProductFamily().getQuoteToLe().getQuote().getId(), serviceDetailBean));
	...
	private QuoteProductComponent constructQuoteProductComponent(QuoteIllSite illSite,
		...
		productComponent.setReferenceName(QuoteConstants.GVPN_SITES.toString());
		quoteProductComponentRepository.save(productComponent);
		...
		try {
			...
			if (orderIllSite != null) {
				...
				if (serviceDetailBean != null) {
					QuoteIllSiteToService illSiteToService = new QuoteIllSiteToService();
					illSiteToService.setErfServiceInventoryTpsServiceId(serviceDetailBean.getServiceId());
					illSiteToService.setTpsSfdcParentOptyId(serviceDetailBean.getOpportunityId() != null ? Integer.valueOf(serviceDetailBean.getOpportunityId()) : null);
					if (MACDConstants.SINGLE_ALL_CAPS.equalsIgnoreCase(serviceDetailBean.getPrimarySecondary())
							|| (MACDConstants.DUAL_PRIMARY.equalsIgnoreCase(serviceDetailBean.getPrimarySecondary())))
						illSiteToService.setType(PDFConstants.PRIMARY);
					else if (MACDConstants.DUAL_SECONDARY.equalsIgnoreCase(serviceDetailBean.getPrimarySecondary()))
		...
		if (ordrProductComponentsAttributeValues != null) {
			for (OrderProductComponentsAttributeValue orderAttributeValue : ordrProductComponentsAttributeValues) {
				...
				case "VPN Port":
					...
					break;
				case "Last mile":
					quoteAttributeValue.setProductAttributeMaster(getProductAttributeMaster(PDFConstants.LOCAL_LOOP_BANDWIDTH));
