...
public class GscPricingFeasibilityService {
	 ...
	 * @return
	 */
	private String setPrecisionForLessValues(Double value, Integer precision) {
		Double result = 0.0;
		if (Objects.nonNull(value)) {
			if (precision == 2) {
				result = Math.round(value * 100.0) / 100.0;
	...
	private GscPricingFeasibilityContext getPricingResponse(GscPricingFeasibilityContext context) {
		try {
			...
			if (pricingResponse.getStatus() == Status.SUCCESS) {
				...
				//GP-396 - Temp fix for Domestic voice pricing
				if(PARTNER.equalsIgnoreCase(userInfoUtils.getUserType())  && PartnerConstants.SELL_THROUGH_CLASSIFICATION.
						equals(context.quoteToLe.getClassification())){
					LOGGER.info("Domestic voice Partner Sell throught Logic - Override OrderSetupNrc and ChannelMrc to :: 0" );
					context.pricingResponse.getDomesticVoicePrice().setOrderSetupNrc("0");
					context.pricingResponse.getDomesticVoicePrice().setChannelMrc("0");
				}
	...
	private void extractServiceWisePriceDetails(GscPricingFeasibilityContext context, QuoteGsc quoteGsc,
		...
		case GscConstants.ACDTFS: {
			...
			break;
		case GscConstants.DOMESTIC_VOICE: {
			this.setDomesticVoicePrices(context, context.pricingResponse.getDomesticVoicePrice(), quoteGsc, quoteGscDetail);
	 ...
	 * @param quoteGscDetail
	 */
	private void setDomesticVoicePrices(GscPricingFeasibilityContext context, DomesticVoicePriceBean configPrice, QuoteGsc quoteGsc, QuoteGscDetail quoteGscDetail) {
		if (!configPrice.getCurrency().equalsIgnoreCase("0")) {
			...
			List<QuoteProductComponent> domesticProductComponents = quoteProductComponentRepository.findByReferenceIdAndMstProductComponent_NameAndType(quoteGscDetail.getId(), GscConstants.DOMESTIC_VOICE, GscConstants.GSC_ORDER_PRODUCT_COMPONENT_TYPE);
			if (!domesticProductComponents.isEmpty()) {
				List<QuoteProductComponentsAttributeValue> quoteProductComponentsAttributeValues = quoteProductComponentAttributeValuesRepository.findByQuoteProductComponent(domesticProductComponents.get(0));
				quoteProductComponentsAttributeValues.stream().forEach(quoteProductComponentsAttributeValue -> {
					if(quoteProductComponentsAttributeValue.getProductAttributeMaster().getName().equalsIgnoreCase(GscConstants.COUNT_REQUESTED_NUMBERS)){
						context.totalDomNumbersRequested = Integer.valueOf(quoteProductComponentsAttributeValue.getAttributeValues());
						LOGGER.info("Total number of dom numbers requested {}", context.totalDomNumbersRequested);
					}
				});
			}

			List<QuoteProductComponent> publicIpProductComponents = quoteProductComponentRepository.findByReferenceIdAndMstProductComponent_NameAndType(quoteGscDetail.getId(), GscConstants.PUBLIC_IP, GscConstants.GSC_ORDER_PRODUCT_COMPONENT_TYPE);
			if (!publicIpProductComponents.isEmpty()) {
				List<QuoteProductComponentsAttributeValue> quoteProductComponentsAttributeValues = quoteProductComponentAttributeValuesRepository.findByQuoteProductComponent(publicIpProductComponents.get(0));
				quoteProductComponentsAttributeValues.stream().forEach(quoteProductComponentsAttributeValue -> {
					if(quoteProductComponentsAttributeValue.getProductAttributeMaster().getName().equalsIgnoreCase(GscConstants.COUNT_CONCURRENT_CHANNELS)){
						context.totalChannelsRequested = Integer.valueOf(quoteProductComponentsAttributeValue.getAttributeValues());
			...
			context.totalMRC = (setPrecision(Double.valueOf(configPrice.getDidMrc()), 2) * context.totalDomNumbersRequested)
					+ (setPrecision(Double.valueOf(configPrice.getChannelMrc()),2) * context.totalChannelsRequested)
					+ Double.valueOf(configPrice.getOrderSetupMrc());
...
//			context.totalNRC = context.totalNRC * didConcurrentChannel;
		...
		if (Objects.nonNull(configPrice.getPrice()) && !configPrice.getPrice().isEmpty()
			...
			for (int i = 0; i < configPrice.getPrice().size(); i++) {
				if (!configPrice.getCurrency().get(i).equalsIgnoreCase("0")) {
					String rpm = configPrice.getPrice().get(i);
					Double priceAfterConversion = this.convertCurrency(configPrice.getCurrency().get(i),
							context.resultantCurrency, Double.parseDouble(rpm));
					if (priceAfterConversion < 0.00094) {
						prices.add(this.setPrecisionForLessValues(priceAfterConversion, 4));
					} else {
						prices.add(this.setPrecision(priceAfterConversion, 4).toString());
					}
	...
	private void updatePriceForDomesticVoice(ManualPricingContext context) {
		if(DOMESTIC_VOICE.equalsIgnoreCase(context.quoteGsc.getProductName())) {
			List<QuoteProductComponent> domesticProductComponents = quoteProductComponentRepository.findByReferenceIdAndMstProductComponent_NameAndType(context.quoteGscDetail.getId(), GscConstants.DOMESTIC_VOICE, GscConstants.GSC_ORDER_PRODUCT_COMPONENT_TYPE);
			if (!domesticProductComponents.isEmpty()) {
				List<QuoteProductComponentsAttributeValue> quoteProductComponentsAttributeValues = quoteProductComponentAttributeValuesRepository.findByQuoteProductComponent(domesticProductComponents.get(0));
				quoteProductComponentsAttributeValues.stream().forEach(quoteProductComponentsAttributeValue -> {
					if (quoteProductComponentsAttributeValue.getProductAttributeMaster().getName().equalsIgnoreCase(GscConstants.COUNT_REQUESTED_NUMBERS)) {
			...
			List<QuoteProductComponent> publicIpProductComponents = quoteProductComponentRepository.findByReferenceIdAndMstProductComponent_NameAndType(context.quoteGscDetail.getId(), GscConstants.PUBLIC_IP, GscConstants.GSC_ORDER_PRODUCT_COMPONENT_TYPE);
			if (!publicIpProductComponents.isEmpty()) {
				List<QuoteProductComponentsAttributeValue> quoteProductComponentsAttributeValues = quoteProductComponentAttributeValuesRepository.findByQuoteProductComponent(publicIpProductComponents.get(0));
				quoteProductComponentsAttributeValues.stream().forEach(quoteProductComponentsAttributeValue -> {
					if (quoteProductComponentsAttributeValue.getProductAttributeMaster().getName().equalsIgnoreCase(GscConstants.COUNT_CONCURRENT_CHANNELS)) {
			...
			double totalMRC = (setPrecision(Double.valueOf(context.pricingRequest.getDidDomesticMrc()), 2) * context.totalDomNumbersRequested)
					+ (setPrecision(Double.valueOf(context.pricingRequest.getDidChannelMrc()), 2) * context.totalChannelsRequested)
					+ (setPrecision(Double.valueOf(context.pricingRequest.getDidOrderSetupMrc()), 2));
			double totalNRC = (setPrecision(Double.valueOf(context.pricingRequest.getDidDomesticNrc()), 2))
					+ (setPrecision(Double.valueOf(context.pricingRequest.getDidChannelNrc()), 2))
					+ (setPrecision(Double.valueOf(context.pricingRequest.getDidOrderSetupNrc()), 2));
			int didConcurrentChannel = getDIDConcurrentChannel(context.quoteGsc);
			context.pricingRequest.setMrc(totalMRC);
			context.pricingRequest.setNrc(totalNRC);
	...
	private void getAttributes(GscPricingFeasibilityContext context, QuoteGsc quoteGsc, QuoteGscDetail gscConfig,
		...
		List<String> attributesList = Arrays.asList(GscConstants.RATE_PER_MINUTE_FIXED,
				GscConstants.RATE_PER_MINUTE_MOBILE, GscConstants.RATE_PER_MINUTE_SPECIAL,GscConstants.RATE_PER_MINUTE, GscConstants.TERM_NAME,
				GscConstants.DID_ARC, GscConstants.DID_MRC, GscConstants.DID_NRC, GscConstants.CHANNEL_ARC,
				GscConstants.CHANNEL_MRC, GscConstants.CHANNEL_NRC, GscConstants.ORDER_SETUP_ARC,
				GscConstants.ORDER_SETUP_MRC, GscConstants.ORDER_SETUP_NRC, GscConstants.TERM_RATE,
	...
	private void getAttributesValues(GscPricingFeasibilityContext context,
		...
		if (!attributeValues.isEmpty()
			...
			case GscConstants.RATE_PER_MINUTE_MOBILE:
				...
				break;
                case GscConstants.RATE_PER_MINUTE:
                    gscConfigurationBean.setRpm(getRpmValues(attributeValues));
                    break;
			case GscConstants.TERM_NAME:
	...
	private void convertGscDetailAttributes(GscPricingFeasibilityContext context)
	{
		context.quoteGscs.stream().forEach(quoteGsc ->{
			quoteGsc.getQuoteGscDetails().stream().forEach(quoteGscDetail -> {
			   ...
			   convertMultipleAttributes(quoteGsc,quoteGscDetail,context,GscConstants.RATE_PER_MINUTE);
	...
	private void setPartnerAttributes(GscPricingFeasibilityContext context) {
		if(Objects.nonNull(context.quoteToLe.getClassification()) &&
				context.quoteToLe.getClassification().equalsIgnoreCase(PartnerConstants.SELL_THROUGH_CLASSIFICATION)){
