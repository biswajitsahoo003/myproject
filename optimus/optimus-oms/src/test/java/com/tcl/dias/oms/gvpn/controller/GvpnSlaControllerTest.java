...
import com.tcl.dias.oms.gvpn.controller.v1.GvpnSlaController;
...
/**
 * 
 * This file contains the GvpnSlaControllerTest.java class.
 * 
 *
...
public class GvpnSlaControllerTest {
	...
	@Autowired
	private GvpnSlaController gvpnSla;

	@Autowired
	private GvpnSlaService gvpnSlaService;
	...
	public void init() {
		...
		Mockito.when(slaMasterRepository.findBySlaName(Mockito.anyString())).thenReturn(null);
		...
		Mockito.when(quoteProductComponentsAttributeValueRepository
				...
				.thenReturn(quoteObjectCreator.createQuoteProductAttributeValueGVPN());
		Mockito.when(quoteProductComponentsAttributeValueRepository
		...
		Mockito.when(quoteToLeProductFamilyRepository.findByQuoteToLe(Mockito.any()))
				.thenReturn(quoteObjectCreator.getQuoteToLeFamilyListGVPN());
		Mockito.when(productAttributeMasterRepository.findById(Mockito.eq(1)))
				.thenReturn(Optional.of(quoteObjectCreator.getProductAtrributePrimary()));
	...
	public void testsaveSla() throws TclCommonException {
		...
		ResponseResource<SlabBean> response = gvpnSla.saveSla(quoteObjectCreator.slaUpdateRequest());
		...
		list.stream().forEach(value -> {
			try {
				...
				Mockito.when(mqUtils.sendAndReceive(Mockito.anyString(), Mockito.eq("Unmanaged,1")))
						.thenReturn(quoteObjectCreator.getProductSlaBeanjsonGVPN());
				gvpnSlaService.saveSla(quoteObjectCreator.getQuoteToLeSaveSla());
	...
	public void testsaveSlaforelse() throws TclCommonException {
		...
		Mockito.when(siteFeasibilityRepository.findByQuoteIllSite_IdAndIsSelected(Mockito.anyInt(), Mockito.anyByte()))
				.thenReturn(quoteObjectCreator.getSiteFeasibilities1());
		when(mqUtils.sendAndReceive(Mockito.anyString(), Mockito.eq("Unmanaged,chennai,GVPN")))
				.thenReturn(quoteObjectCreator.getProductSlaBeanjsonGVPN());
		gvpnSlaService.saveSla(quoteObjectCreator.getQuoteToLeSaveSlaForElse());
	}
	...
	public void testsaveSlaForProductFamilyEmpty() throws TclCommonException {
		...
		Mockito.when(leProductSlaRepository.findByErfCustomerIdAndMstProductFamily(Mockito.anyInt(), Mockito.any()))
				.thenReturn(null);
		gvpnSlaService.saveSla(quoteObjectCreator.getQuoteToLe());
