...
public class GscOrderControllerTest {
	...
	@Test
	public void testGenerateCofPdf() throws IOException {
		...
		Mockito.when(quoteToLeRepository.findByQuote(Mockito.any())).thenReturn(gscObjectCreator.getQuoteToLeList());
		Mockito.when(cofDetailsRepository.findByOrderUuid(Mockito.any())).thenReturn(gscObjectCreator.getCofDetails());
		Mockito.when(productSolutionRepository.findByQuoteToLeProductFamily(Mockito.any()))
				.thenReturn(gscObjectCreator.getProductSolutionList());
		Mockito.when(quoteToLeProductFamilyRepository.findByQuoteToLe_Id(Mockito.anyInt()))
				.thenReturn(gscObjectCreator.getQuoteToLeProductFamily());
		Mockito.when(httpServletResponse.getOutputStream()).thenReturn(new ServletOutputStream() {
		...
		String response = gscQuotePdfService.processCofPdf(1, httpServletResponse, false, false);
		assertNotNull(response);

		String contextNotApprovedResponse = gscQuotePdfService.processCofPdf(1, httpServletResponse, false, true);
		assertNotNull(contextNotApprovedResponse);

		Mockito.when(cofDetailsRepository.findByOrderUuid(Mockito.any())).thenReturn(null);

		String contextNotApprovedResponseWithNullCofDetails = gscQuotePdfService.processCofPdf(1, httpServletResponse,
				null, true);
		assertNotNull(contextNotApprovedResponseWithNullCofDetails);
	}
