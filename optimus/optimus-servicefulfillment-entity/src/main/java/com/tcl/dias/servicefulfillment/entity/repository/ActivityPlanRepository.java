...
public interface ActivityPlanRepository extends JpaRepository<ActivityPlan, Integer> {
	...
	@Query(value = "SELECT * FROM activity_plan where service_id=:serviceId and activity_def_key=:defkey and site_type=:siteType", nativeQuery = true)
    ActivityPlan findByServiceIdAndMstActivityDefKey(@Param("serviceId") Integer serviceId,@Param("defkey") String defkey, String siteType);
	
	@Query(value = "SELECT * FROM activity_plan where service_id=:serviceId and activity_def_key=:defkey and site_type=:siteType order by id desc limit 1", nativeQuery = true)
    ActivityPlan findFirstByServiceIdAndMstActivityDefKey(@Param("serviceId") Integer serviceId,@Param("defkey") String defkey, String siteType);
