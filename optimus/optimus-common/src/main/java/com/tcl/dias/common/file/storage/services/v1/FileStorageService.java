package com.tcl.dias.common.file.storage.services.v1;
...
public class FileStorageService {
	...
	private Container getStorageContainer(String customerCode, String customerLeCode, String secsCode) {
		Account account = getAccount();
		String path = swiftApiContainer + CommonConstants.HYPHEN + customerCode + CommonConstants.HYPHEN
				+ customerLeCode + CommonConstants.HYPHEN + secsCode;
		Container myContainer = account.getContainer(path);
		if (!myContainer.exists()) {
			myContainer.create();
			myContainer.makePublic();
	...
	public StoredObject uploadGscObject(String uniqueName, InputStream inputStream, String customerCode,
		...
		try {
			Container container = getStorageContainer(customerCode, customerLeCode);
			String requestId = UUID.randomUUID().toString() + CommonConstants.UNDERSCORE + uniqueName;
	...
	/**
	 * Upload Any kind of document with secs id value
	 *
	 * @param inputStream
	 * @param customerCode
	 * @param customerLeCode
	 ...
	 * @throws TclCommonException
	 */
	public StoredObject uploadObjectWithSecsId(InputStream inputStream, String customerCode,
									 String customerLeCode, String secsCode) throws TclCommonException {
		StoredObject storedObject;
		try {
			Container container = getStorageContainer(customerCode, customerLeCode, secsCode);
			String requestId = UUID.randomUUID().toString();
			storedObject = container.getObject(requestId);
			if (storedObject.exists()) {
				throw new TclCommonException(ExceptionConstants.OBJECT_ALREADY_EXISTS, ResponseResource.R_CODE_ERROR);
	...
	/**
	 * download PDF in Object Storage as IS
	 *
	 * @param fileName
	 * @param expiryWindowInSeconds
	 * @param containerUrl
	 * @param isInternal
	 * @return InputStream
	 * @throws TclCommonException
	...
	public InputStream downloadFileAsInputStream(String fileName, long expiryWindowInSeconds, String containerUrl,
			boolean isInternal) throws TclCommonException {
		...
		try {
			...
			if (container.exists()) {
				...
				if(newObject.exists())
					inputStream = newObject.downloadObjectAsInputStream();
			}
			return inputStream;
		} catch (Exception e) {
			throw new TclCommonException(ExceptionConstants.COMMON_ERROR, e, ResponseResource.R_CODE_ERROR);
	...
	/**
	 * Method to upload object  in storedObject
	 * @param uniqueName
	 * @param inputStream
	 * @return
	 * @throws TclCommonException
	 */
	public StoredObject uploadAttachements(String uniqueName, InputStream inputStream) throws TclCommonException {
		StoredObject storedObject = null;
		try {
			LOGGER.info("Entering fileStorageService.uploadAttachements to upload attachement {} ",uniqueName);
			Container container = getStorageContainer("SUPPORT_MATRIX_ATTACHMENT");
	...
	private Container getStorageContainerSDD(String containerName) {
		Account account = getAccount();
		String path = swiftApiContainer + CommonConstants.HYPHEN + containerName ;
		Container myContainer = account.getContainer(path);
		if (!myContainer.exists()) {
			myContainer.create();
			myContainer.makePublic();
	...
	public StoredObject uploadObjectSDD(String uniqueName, InputStream inputStream, Integer referenceId
			) throws TclCommonException {
		StoredObject storedObject = null;
		try {
			String containerName=CommonConstants.IZOSDWAN_QUOTES;
			Container container = getStorageContainerSDD(containerName);
			String requestId = UUID.randomUUID().toString()+""+uniqueName;
			storedObject = container.getObject(requestId);
			if (storedObject.exists()) {
				throw new TclCommonException(ExceptionConstants.OBJECT_ALREADY_EXISTS, ResponseResource.R_CODE_ERROR);
	...
	public TempUploadUrlInfo getTempUploadUrlWithFileName(long expiryWindow, String customerCode, String customerLeCode,
			boolean isInternal, String fileName) throws TclCommonException {
		try {
			Container container = getStorageContainer(customerCode, customerLeCode);
			String requestId = UUID.randomUUID().toString() + fileName;
			String temporaryUrl;
	...
	public StoredObject uploadOverwriteeObjectWithFileName(String fileName, InputStream inputStream, String customerCode,
			String customerLeCode) throws TclCommonException {
		StoredObject storedObject = null;
		try {
			Container container = getStorageContainer(customerCode, customerLeCode);
