...
//import com.tcl.dias.servicefulfillment.config.KeycloakInterceptor;
import com.tcl.dias.servicefulfillment.controller.v1.RfLmImplementationController;
...
public class RfLmImplementationControllerTest extends Thread{
	
	@MockBean
	CreateServiceDelegate createServiceDelegate;
	
	@Autowired
	RfLmImplementationController rfLmImplementationController;
...
//
		...
		List<Task> taskList4PW = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"provide-wo-rf-site-survey");
		if(!taskList4PW.isEmpty()) {
			Integer taskId4 = taskList4PW.get(0).getId();

			sleep(1000);
		
			HttpEntity<WoSiteSurveyBean> requestEntity3 = new HttpEntity<WoSiteSurveyBean>(objectCreator.createWoSiteSurveyBean());
			ResponseEntity<ResponseResource> responseEntity3 = restTemplate.postForEntity("/v1/rf-lm-implementation/task/provide-wo-rf-site-survey/" + taskId4, requestEntity3, ResponseResource.class);
		}
			
//		public void provideWoSiteSurvey() throws TclCommonException {
//			doNothing().when(flowableTaskService).complete(Mockito.anyString(), Mockito.any());
//			ResponseResource<WoSiteSurveyBean> response = rfLmImplementationController.provideWoSiteSurvey(1, objectCreator.createWoSiteSurveyBean());
...
//		}
		...
		List<Task> taskList5 = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"arrange-field-engineer-rf-ss");
		...
		// Task : rf Conduct-Site Survey

				List<Task> taskList5SS = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"lm-rf-conduct-site-survey");
				if(!taskList5SS.isEmpty()) {
					Integer taskId5 = taskList5SS.get(0).getId();

					sleep(1000);

					HttpEntity<ConductRfSiteSurveyBean> requestEntity4 = new HttpEntity<ConductRfSiteSurveyBean>(objectCreator.createConductRfSiteSurveyBean());
					ResponseEntity<ResponseResource> responseEntity4 = restTemplate.postForEntity("/v1/rf-lm-implementation/task/conduct-site-survey/" + taskId5, requestEntity4, ResponseResource.class);
				}
		...
		// Task : Create Rf MRN
				...
				List<Task> taskList5MRN = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"lm-create-rf-mrn");
				if(!taskList5MRN.isEmpty()) {
					Integer taskId5 = taskList5MRN.get(0).getId();

					sleep(1000);

					HttpEntity<CreateMrnBean> requestEntity4 = new HttpEntity<CreateMrnBean>(objectCreator.createCreateMrnBean());
					ResponseEntity<ResponseResource> responseEntity4 = restTemplate.postForEntity("/v1/rf-lm-implementation/task/rf-mrns/" + taskId5, requestEntity4, ResponseResource.class);
				}
				
		// Task : Conduct Survey

		List<Task> taskList6 = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"deliver-rf-equipment");
		if(!taskList6.isEmpty()) {
			Integer taskId6 = taskList6.get(0).getId();

			sleep(1000);

			HttpEntity<DeliverRfEquipmentDetails> requestEntity5 = new HttpEntity<DeliverRfEquipmentDetails>(objectCreator.createDeliverRfEquipmentDetails());
			ResponseEntity<ResponseResource> responseEntity5 = restTemplate.postForEntity("/v1/rf-lm-implementation/task/deliver-rf-equipment/" + taskId6, requestEntity5, ResponseResource.class);
		}
...
//
///**
//* Test Cases for RF LM Implementation Controller
//* 
//*
//* @author diksha garg
...
//@AutoConfigureMockMvc
//@FixMethodOrder(MethodSorters.NAME_ASCENDING)
//public class RfLmImplementationControllerTest {
//	
//	@Autowired
//	RfLmImplementationController rfLmImplementationController;
//	
...
//	
//	/**
//	 * Positive test case for provide work order for site survey
//	 * 
//	 * provideWoSiteSurvey
//	 * @throws TclCommonException
//	 */
...
//
//  @Test
//  public void enrichRfDetails() {
//		rfService.enrichRfDetails("000.000.000");
//  }
//}
//
