...
public class EnrichmentControllerTest extends Thread{
...
//		// Task : Provide Appointment for Site Survey
		...
		List<Task> taskList3 = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"customer-appointment-ss");
...
//z
		List<Task> taskList4 = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"select-vendor-ss");
...
//        log.info("WfProcessInstId={} executionId={}", taskProcessInstID, execution.getId());
		...
		//Get Mux info Async
		List<Task> taskList24A = taskRepo.findByServiceIdAndMstTaskDef_key(serviceId,"get-mux-info-async");
		if(!taskList24A.isEmpty	()) {
			String taskProcessInstID = taskList24A.get(0).getWfProcessInstId();

		 Execution execution = runtimeService.createExecutionQuery().processInstanceId(taskProcessInstID).activityId("get-mux-info-async").singleResult();
         if (execution != null) {
//        log.info("WfProcessInstId={} executionId={}", taskProcessInstID, execution.getId());
             runtimeService.trigger(execution.getId());
         }
