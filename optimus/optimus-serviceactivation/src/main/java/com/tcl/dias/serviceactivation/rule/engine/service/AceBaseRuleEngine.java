...
public abstract class AceBaseRuleEngine {

	private static final Logger logger = LoggerFactory.getLogger(AceBaseRuleEngine.class);
	...
	public Map<String, String> findMandatoryValues(OrderDetail orderDetail, ServiceDetail serviceDetails) {
		logger.info("AceBaseRuleEngine findMandatoryValues method invoked for serviceId::{}",serviceDetails.getId());
	...
	protected String findVrfDetails(ServiceDetail serviceDetails) {
		if(serviceDetails.getVrfs()!=null) {
			...
			if (mandatoryFields.containsKey(AceConstants.ROUTER.ROUTER_MAKE) && mandatoryFields
				...
				if (serviceQo != null) {
					...
					if (serviceQo.getQosTrafiicMode() != null
							...
							logger.info("Service Cos Criteria Policy Scheduler"); 
					...
					if(serviceQo.getIsflexicos()!=null && serviceQo.getIsflexicos()==1 && 
							serviceDetails.getOrderDetail().getCustomerName()!=null && !serviceDetails.getOrderDetail().getCustomerName().isEmpty()
							&& serviceDetails.getTopologies() != null
							&& !serviceDetails.getTopologies().isEmpty()) {
						...
						if (topology != null && topology.getUniswitchDetails() != null
							...
							UniswitchDetail uniswitchDetail = topology.getUniswitchDetails().stream().findFirst()
									.orElse(null);
							if (uniswitchDetail != null && uniswitchDetail.getUniquePortId() != null
									&& !uniswitchDetail.getUniquePortId().isEmpty()) {
								logger.info("UniSwitch Port exists");
								aluSchedulerPolicy.setSchedulerPolicyName(AceConstants.ACE.ACE_F_
										+ serviceDetails.getOrderDetail().getCustomerName()
												.replaceAll("[^a-zA-Z0-9]", "").substring(0, 8)
										+ "_" + uniswitchDetail.getUniquePortId());
								aluSchedulerPolicy.setSapIngressPolicyname(AceConstants.ACE.ACE_F_IN
										+ serviceDetails.getOrderDetail().getCustomerName()
										.replaceAll("[^a-zA-Z0-9]", "").substring(0, 8)
										+ "_" + uniswitchDetail.getUniquePortId());
								aluSchedulerPolicy.setSapEgressPolicyname(AceConstants.ACE.ACE_F_OUT
										+ serviceDetails.getOrderDetail().getCustomerName()
										.replaceAll("[^a-zA-Z0-9]", "").substring(0, 8)
										+ "_" + uniswitchDetail.getUniquePortId());
								aluSchedulerPolicy.setTotalPirBw(serviceQo.getSummationOfBw());
								aluSchedulerPolicy.setTotalCirBw(serviceQo.getSummationOfBw());
							}
						}
					}
				}
	...
	protected String splitProjectName(String customerName,String projectName) {
		...
		if(projectName.contains(" ")){
			...
			if(projectName.length()>7){
				buffer.append(projectName.substring(0,7));
			}
			else{
