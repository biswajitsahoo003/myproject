...
public interface TaskPlanRepository extends JpaRepository<TaskPlan, Integer> {
	...
	@Query(value = "SELECT * FROM task_plan where service_id=:serviceId and task_def_key=:defkey", nativeQuery = true)
    TaskPlan findByServiceIdAndMstTaskDefKey(@Param("serviceId") Integer serviceId,@Param("defkey") String defkey);
